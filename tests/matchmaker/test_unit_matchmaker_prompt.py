...
def test_sections_appear_only_when_they_apply(has_datasets: bool, allow_magic: bool):
    prompt = _decompose_prompt(has_datasets, allow_magic)
...
def test_plain_prompt_is_the_base_prompt():
    from ap_management.services.matchmaker.matchmaker import DECOMPOSE_PROMPT

    assert _decompose_prompt(False, False) == DECOMPOSE_PROMPT
